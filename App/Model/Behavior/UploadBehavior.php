...
//			App::uses('CakeNumber', 'Utility');
			return __('File size too large. Max %s.', Number::toReadableSize($size));
		}
		return $ret;
...
// Modify for NetCommons Extentions By Ryuji.M --START
		...
		if(!$ret) {
//			App::uses('CakeNumber', 'Utility');
			return __('File size too large. Min %s.', Number::toReadableSize($size));
		}
		return $ret;
 ...
 * @return string
 **/
	public function _path(Model $model, $fieldName, $options = []) {
		$defaults = array(
			'isThumbnail' => true,
			'path' => '{ROOT}webroot{DS}files{DS}{model}{DS}{field}{DS}',
		...
		if (($isImage || $isMedia) && $createThumbnails && $hasThumbnails) {
			...
			foreach ($this->settings[$model->alias][$field]['thumbnailSizes'] as $size => $geometry) {
				...
				if (method_exists($model, $method)) {
					...
					Log::error(sprintf('Model %s, Field %s: Invalid thumbnailMethod %s', $model->alias, $field, $filePath));
					$db = $model->getDataSource();
					$db->rollback();
	...
	public function _prepareFilesForDeletion(Model $model, $field, $data, $options) {
		...
		if (!empty($options['fields']['dir']) && isset($data[$model->alias][$options['fields']['dir']])) {
			...
			if (in_array($options['pathMethod'], array('_getPathFlat', '_getPathPrimaryKey'))) {
				...
				Log::error(sprintf('Cannot get directory to %s.%s: %s pathMethod is not supported.', $model->alias, $field, $options['pathMethod']));
			}
		}
