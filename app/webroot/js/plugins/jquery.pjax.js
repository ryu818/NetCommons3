...
//Edit Start Ryuji.M
    ...
    if(container.top_attributes != null) {
    	// 属性情報更新
    	var attrs = context[0].attributes;
    	for (var i = 0; i <= attrs.length - 1; i++) {
    		var a = attrs.item(i);
    		var name = a.nodeName.toLowerCase();
    		var val = $(context).attr(name);
    		if(container.top_attributes[name]) {
    			$(context).attr(name, container.top_attributes[name]);
    		} else {
...
function extractContainer(data, xhr, options) {
  // Edit Start Ryuji.M
  // 内部にあるscriptが正常に動作しなくなるため、
  // （BackボタンをクリックしてもScriptが動作されるべき）
  var obj = {
    'title':'',
    'contents':'',
  ...
  for (var i = 0; i <= div.childNodes.length - 1; i++) {
  	if(div.childNodes[i].nodeType == 1 && div.childNodes[i].tagName.toLowerCase() == 'title') {
  		// lastではなくfirstのelementにしている
  		obj.title = $.trim($(div.childNodes[i]).text());
  		break;
  	}
  }
  if (options.fragment) {
...
function enable() {
  ...
  $.pjax.defaults = {
    timeout: 0,		// timeoutなしに変更 Ryuji.M
    push: true,
    replace: false,
    type: 'GET',
    dataType: 'html',
