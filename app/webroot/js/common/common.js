 ...
 * data-ajax-callback: string ajax success時 callback関数　thisは設定したelement自身, e:event res:response string
 *		"$(this).html(res); return false;"等
 * [カスタムイベント] ajax:
 ...
 */
		ajax: function(e, el, params, options, _confirm, _force_url) {
			...
			if($el.hasClass('nc-disable-lbl') && !_force_url) {
 ...
 * 			function callback  : 決定 or 記事に挿入時コールバック関数。 引数 fileName, URL, サムネイルURL
 * @return  void
 */
		showUploadDialog : function(id, options) {
			...
			var url = null, pos, params, dialog_title, dialogOptions, dialog;
			...
			if(el) {
				pos = el.offset();
				dialogOptions.position = [pos.left, pos.top + el.outerHeight() - $(window).scrollTop()];
			}
			dialog = $('#'+id);
 ...
 **/
 		...
 		showErrorDialog : function(error_str, params, target) {
			var ok = __('Ok');
			var body = '<div class="nc-error-message">' + error_str + '</div>', _buttons = {}, pos;
		...
		closeAlert : function(input, alert) {
			...
			$.each( $(input), function() {
				...
				child.addClass('nc-error-input-message');
				if(child.is(':input,textarea')) {
					if(child.is(':radio,:checkbox,:submit,:image,:reset,:button,:hidden')) {
						...
						child.bind("keyup change", function(e){
							...
							if(child.val() != text) {
								alert.remove();
								child.removeClass('nc-error-input-message');
							}
						});
					}
				}
 ...
 */
		sliderAuthority: function(id, options, minAuthorityId, maxAuthorityId) {
			...
			if(options['disabled'] == true) {
				// optionsでdisabledを指定するとスタイルが半透明にならないためmethod指定
 ...
 **/
		...
		// setFlashでレスポンスが返ってきたかどうか。
		// 現状、先頭scriptでしか判断していない。
		isSetFlash: function(res) {
			var reHtml = new RegExp("^<script>", 'i');
			return $.trim(res).match(reHtml);
		}
	};
	/* 定数 */
	$.support.pjax =
	...
	__d = function(key, name) {
		...
		if(!ret) {
			if(name == undefined) {
				return (typeof $._lang[key] !== 'undefined') ? $._lang[key] : null;
			}
			ret = (typeof $._lang[key] !== 'undefined' && $._lang[key][name]) ? $._lang[key][name] : name;
		}
		if(arguments.length > 2) {
			r.push.apply(r, arguments);
			r.shift();
