...
<?php foreach ($blog_comments_tree as $blogComments) { ?>
	<?php
		...
		if(!empty($blogComments['BlogComment']['author_url'])) {
			$authorHtml = $this->Html->link(h($author), $this->MyHtml->linkUrl($blogComments['BlogComment']['author_url']),
				array('id' => 'comment-author-link'.$id.'-'.$blogComments['BlogComment']['blog_post_id'],
					'class' => 'nc-tooltip', 'title' => $this->MyHtml->linkUrl($blogComments['BlogComment']['author_url']),'target' => '_blank'
				));
		}else{
			$authorHtml = h($author);
		}
