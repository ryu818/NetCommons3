		...
		foreach($tables as $table) {
			...
			foreach($datas as $data) {
				...
				if($table == 'Revision') {
					...
					$data['BlogComment']['blog_post_id'] = $postId;
					// lftとrghtの自動設定をやめるためにTreeBehaviorを無効化
					if ($this->BlogComment->Behaviors->enabled('Tree')) {
						$this->BlogComment->Behaviors->disable('Tree');
					}
			...
			if($table == 'BlogPost') {
				$postId = $this->{$table}->id;
			} else if($table == 'BlogComment') {
				if (!$this->BlogComment->Behaviors->enabled('Tree')) {
					$this->BlogComment->Behaviors->enable('Tree');
				}
				$this->BlogComment->Behaviors->attach('Tree', array(
					'scope' => array('BlogComment.blog_post_id' => $postId)
				));
				// lft,rghtの値を元に新しいparent_idの値を設定
				$this->BlogComment->recover('tree');
			}
		}
		return true;
	}
