 ...
 * @return  void
 */
		init: function(id, dialogId, setting, activeTab, fileinfoOptions) {
			var tabs = $('#' + id);
			var dialog = $('#' + dialogId);
				...
				if(res['search_results']) {
					...
					$.each(res['search_results'], function(){
						...
						if($.Upload.data[uploadId] && $.Upload.data[uploadId]['_selected']) {
							li.addClass($.Upload.data[uploadId]['_selected']);
						}
						libraryUl.append(li);
					});
 ...
 */
		initRefUrlTab: function(id) {
			...
			if ($img) {
				...
				if(imgObject.width > 0) {
					data['resize_width'] = data['width'];
					data['resize_height'] = data['height'];
					data['width'] = imgObject.width;
					data['height'] = imgObject.height;
				}
 ...
 * @return  void
 */
		changeSearchUserType: function(select, id, myself) {
			if($(select).val() != myself) {
				$('#upload-text-options' + id).show();
				$('#upload-created-all' + id).show();
				$('#upload-created-all' + id).prev().hide();
			} else {
				$('#upload-text-options' + id).hide();
				$('#upload-created-all' + id).hide();
				$('#upload-created-all' + id).prev().show();
			}
 ...
 */
		changeRefUrl: function(input) {
			...
			if(imgObject.width > 0) {
				...
				$.Upload.data[0]['width'] = $.Upload.data[0]['resize_width'] = imgObject.width;
				$.Upload.data[0]['height'] = $.Upload.data[0]['resize_height'] = imgObject.height;
				$('#' + this.refUrlId + '-fileinfo-percent-size').val(100);
				$('#' + this.refUrlId + '-fileinfo-px-size-width').val(imgObject.width);
				$('#' + this.refUrlId + '-fileinfo-px-size-height').val(imgObject.height);
