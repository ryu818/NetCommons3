...
if(empty($auth[$authority_id]['authority_name'])) {
	$authority_name = $auth[$def_authority_id]['authority_name'];
} else {
	$authority_name = $auth[$authority_id]['authority_name'];
}
...
if(count($auth) == 1 || $selauth == false) {
	$select_html = "<span class=\"pages-menu-auth-listbox-name\">".h(__($authority_name))."</span>";
	$select_after_html = '';
} else {
	$select_html = '';
	$select_after_html = "<select class=\"pages-menu-auth-listbox-name\" onchange=\"$.PageMenu.chgSelectAuth(".$page['Page']['id'].", this); return false;\" >";
		foreach($auth as $data) {
			...
			$select_after_html .= "<option value=\"".$data['id']."\"".$selected.">".h(__($data['authority_name']))."</option>";
		}
...
if($radio) {
	...
	$hidden_detail_html = "";
	if($active_user_id == $user_id && $admin_hierarchy < NC_AUTH_MIN_ADMIN) {
