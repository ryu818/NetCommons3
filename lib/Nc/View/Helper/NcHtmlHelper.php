...
class NcHtmlHelper extends HtmlHelper {
/**
 * instance
 *
 * @var array
 */
	...
	public function __construct(View $View, $settings = array()) {
        ...
        if ($instance === NULL) {
        	$instance = new NcHtmlHelperInstance($View, $settings, $this);
        }
...
class NcHtmlHelperInstance extends AppHelper {
/**
		...
		foreach($paths_arr as $path) {
			...
			if ($rel == 'import') {
				...
				if(substr($path, -1) == '/' || substr($path, -1) == '.') {
					list($url, $plugin, $realPath) = $this->ncAssetUrl($path, $options + array('pathPrefix' => CSS_URL));
				} else {
					list($url, $plugin, $realPath) = $this->ncAssetUrl($path, $options + array('pathPrefix' => CSS_URL, 'ext' => '.css'));
				}
		...
		foreach($urls_arr as $url) {
			...
			if (strpos($url, '//') === false) {
				if(substr($url, -1) == '/' || substr($url, -1) == '.') {
					list($url, $plugin, $realPath) = $this->ncAssetUrl($url, $options + array('pathPrefix' => JS_URL));
				} else {
					list($url, $plugin, $realPath) = $this->ncAssetUrl($url, $options + array('pathPrefix' => JS_URL, 'ext' => '.js'));
				}
 ...
 */
	public function ncAssetUrl($path, $options = array()) {
