 ...
 * @param   integer participant_page_id 参加者修正画面を表示するpage_id
 * @return  void
 * @since   v 3.0.0.0
 */
	$.fn.PageMenu = function(is_edit, active_page_id, active_tab, sel_active_tab, copy_page_id, participant_page_id) {
		// Privateメソッド
		...
		var dletePage = function(url, page_id, all_delete, params) {
			$.post(url, {'data[Page][id]': page_id, 'all_delete': all_delete, 'token': $('#pages-menu-token').val()},function(res){
		...
		// スクロール
		setTimeout(function(){
			$.PageMenu.slideTarget(active_li, active_tab_name);
		}, 500);
		...
		.on('change', function(e, page_id, drop_page_id, position) {
			...
			var data = {'data[Page][id]': page_id, 'data[DropPage][id]': drop_page_id, 'position': position, 'token': $('#pages-menu-token').val()};
			var ret = null;
		...
		if(is_edit) {
			...
			$('#pages-menu-add-btn').on('ajax:beforeSend', function(e, url) {
				...
				var ajax_id, page_edit, replace_ajax_id, active, dd_sequence, position, url, data = {};
				...
				// スクロール
				$.PageMenu.slideTarget(target, active_tab_name);
				// コンテンツクリックイベント
				$('.pages-menu-edit-content:first', target).click();
				if(copy_page_id > 0) {
			...
			root_menu.add('#pages-menu-edit-other-operation').on('ajax:beforeSend','[data-page-edit-id]',function(e, url) {
				...
				// スクロール
				$.PageMenu.slideTarget(target, active_tab_name, scroll_target);
			...
			root_menu.on('ajax:beforeSend','form.pages-menu-edit-form',function(e, url) {
				...
				var page_id = li.attr('data-id');
				var reReloadHtml = new RegExp("^<div id=\"pages-menu-edit-participant-"+page_id+"\"", 'i');
				res = $.trim(res);
				if($(this).attr('data-name') == 'participant' && res != '' && !res.match(re_html)) {
					if(res.match(reReloadHtml)) {
						// reload
						$("#pages-menu-edit-participant-" + page_id).replaceWith(res);
					} else {
			...
			root_menu.on('click','.pages-menu-display-flag',function(e) {
				...
				$.post(url, {'data[Page][id]': page_id, 'data[Page][display_flag]': setDisplay, 'token': $('#pages-menu-token').val()});
			});
		...
		if(participant_page_id) {
			// 編集->参加者修正ボタンによるSubmitの場合、参加者修正画面に遷移。
			$.PageMenu.showEntryMembers(participant_page_id);
		}
	};
	$.PageMenu ={
 ...
 * @param   integer active_tab ａｃｔｉｖｅなタブをセット:default 0
 * @return  void
 */
 ...
 * @param   string  preUrl
 * @param   string  url
 * @param   boolean is_participant 参加者修正へ遷移するかどうか
 * @return  void
 * @since   v 3.0.0.0
 */
		itemInit: function(page_id, preUrl, url, is_participant) {
			...
			if(replaceUrl != location.href && (replaceUrl == '' || replaceUrl.substr(0, 1) == '/' || replaceUrl.substr(0, 1) == '?')) {
				...
				if(is_participant) {
					isParticipant = '&participant_page_id=' + page_id;
				}
				location.href = url + '?is_edit=1' + isParticipant;
			} else if(is_participant) {
				// 編集->参加者修正ボタンによるSubmitの場合、参加者修正画面に遷移。
				$.PageMenu.showEntryMembers(page_id);
			}
		},
 ...
 * @since   v 3.0.0.0
 */
		showEntryMembers: function(page_id) {
			var content = $('.pages-menu-edit-content:first', $('#pages-menu-edit-item-' + page_id));
			var list = $('#pages-menu-edit-other-operation');
			list.attr('data-id', page_id);
 ...
 * @return  void
 */
		selectCommunityFile: function(page_id, isUpload, fileName, url, libraryUrl) {
			var img = $('#pages-menu-community-photo-' + page_id);
			...
			$('#pages-menu-community-photo-hidden-' + page_id).val(fileName);
			$('#pages-menu-community-is-upload-hidden-' + page_id).val(isUpload);
		},

/**
 * 画面スライド
 * @param   element target
 * @param   element active_tab_name
 * @param   element  scroll_target
 * @return  void
 */
		slideTarget: function(target, active_tab_name, scroll_target) {
 ...
 */
		communityOperationCallback: function(e, res) {
			...
			if($.trim(res).match(reHtml)) {
				$('#nc-community-inf').html(res).dialog("destroy");
				return false;
			}
			return true;
		}
