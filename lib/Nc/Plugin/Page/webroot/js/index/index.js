	...
	$.fn.PageMenu = function(is_edit, active_page_id, active_tab, sel_active_tab, copy_page_id) {
		...
		if(is_edit) {
			...
			root_menu.on('ajax:beforeSend','form.pages-menu-edit-form',function(e, url) {
				...
				addEventNestable(active_tab_name, root_menu, target, page_id);
				$.Event(e).preventDefault();
				if(res.match(re_html) && $(this).attr('data-is-participant') == '1' && $(this).attr('data-name') != 'participant') {
 ...
 */
		communityDetailInit : function(id, active_tab) {
			...
			$('input[name="data[Community][publication_range_flag]"]', tab).change(function(e){
				var target = $(this);
				var participateAsGeneral = $('#pages-menu-community-participate-as-general-outer-' + id);
				if(target.val() == '0') {
					participateAsGeneral.slideUp();
				} else {
					participateAsGeneral.slideDown("slow", function() {participateAsGeneral.effect("highlight");});
				}
			});
