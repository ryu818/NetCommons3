 ...
 */
	$.fn.PageMenu = function(isEdit, activePageId, activeTab, selActiveTab, isPost, copyPageId, participantPageId) {
		...
		// コミュニティーはすべて閉じた状態で表示
		$(rootMenu.get(1)).nestable('collapseAll', ['.highlight:first']);
 ...
 */
		clkOtherOperation : function(e) {
			...
			if(copyIsTop != isTop) {
				$('a:first', copyAfter).addClass('disable-lbl');
			} else if((!isTop || (spaceType == 4 && spaceType == copySpaceType)) && isParentChief) {	// 固定値
				$('a:first', copyAfter).removeClass('disable-lbl');
			} else {
				$('a:first', copyAfter).addClass('disable-lbl');
			}
			if(isSelMembers) {
				$('a:first', unassignMembers).removeClass('disable-lbl');
				$('a:first', members).removeClass('disable-lbl');
				$('a:first', addMembers).removeClass('disable-lbl');
			} else {
				$('a:first', unassignMembers).addClass('disable-lbl');
				$('a:first', members).addClass('disable-lbl');
				$('a:first', addMembers).addClass('disable-lbl');
			}
			if(isSelModules) {
