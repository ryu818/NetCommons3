		...
		<div>
			<?php
				...
				if(isset($failure_datas) && $failure_datas) {
					...
					echo '<p class="error nc-message-table">' . __d('install', 'Could not add data of database tables.') . $failure_sql . '</p>';
				}

				if(isset($not_tables) && count($not_tables) > 0) {
					foreach($not_tables as $not_table) {
						//$check = false; // 既に作成済かもなので、エラーとはしない。
						echo '<p class="error nc-message-table">' . __d('install', 'Could not create database %s.', h($not_table)) . '</p>';
					}
				}

				if(isset($tables) && count($tables) > 0) {
					foreach($tables as $table) {
						echo '<p class="success nc-message-table">' . __d('install', 'Database %s created!', h($table)) . '</p>';
					}
				}
				if(isset($failure_install_ini)) {
					if(!$failure_install_ini) {
						echo '<p class="success nc-message-table">' . __d('install', 'install.inc.php file is writing!') . '</p>';
					} else {
						$check = false;
						echo '<p class="error nc-message-table">' . __d('install', 'Could not write install.inc.php file.') . '</p>';
					}
				}
