...
;(function($) {
	$.Authority = {
		...
		initDetail: function(id, myportalMembers) {
			...
			var myportalUseFlag = $('input[name="data[Authority][myportal_use_flag]"]', top);
			var privateUseFlag = $('input[name="data[Authority][private_use_flag]"]', top);
			...
			form.on('ajax:beforeSend',function(e, url) {
				if(!url.match(/\/usable_module\//) || myportalUseFlag.filter(':checked').val() > 0 || privateUseFlag.filter(':checked').val() > 0) {
					return true;
				}
