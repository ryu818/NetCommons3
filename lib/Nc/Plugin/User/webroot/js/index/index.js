...
;(function($) {
	$.fn.User = function(id, list) {
		...
		var colModel = [
			...
			{display: list[1], name : 'UserItemLinkUsername.content', width: 110, height: 26, sortable : true},
	...
	$.User = {
		...
		editInit : function(id, editUserId, editTitle, addUrl) {
			...
			if(editTitle != '') {
				...
				var a = $('a:first', li);
				li.after( $( "<li><a href='"+addUrl+"'>" + a.html() + "</a></li>" ) );
				tabs.tabs( "refresh" );
				a.html(editTitle);
 ...
 */
					...
					lists.each(function(kList, vList){
						...
						cols.each(function(kCol, vCol){
							...
							rows.each(function(kRow, vRow){
								...
								data[i] = new Object();
								data[i]['UserItem'] = new Object();
								data[i]['UserItem']['id'] = itemId;
								data[i]['UserItem']['list_num'] = listNum;
								data[i]['UserItem']['col_num'] = colNum;
								data[i]['UserItem']['row_num'] = kRow + 1;
								i++;
							});
						});
		...
		memberEdit : function(e, a) {
			...
			if($.User.editIds[userId]) {
				...
				ul.children(':first').after( $( "<li><a href='"+$(a).attr('href')+"'>" + title + "</a>"+$.User.getCloseBtn(userId)+"</li>" ) );
				tabs.tabs( "refresh" );
			...
			form.on('ajax:beforeSend',function(e, url) {
				...
				select.children().each(function(){
					//if(!$(this).prop('disabled')) {
						keyArr = $(this).val().split("_");
						ret['data']['PageUserLink'][$(this).val()]= {'room_id' : keyArr[keyArr.length - 1]};
					//}
				});
				return ret;
			});
 ...
 */
		clickGroupOption: function(option) {
			...
			var selects = null;
			var keyArr = option.val().split("_");
			var roomId = keyArr[keyArr.length - 1];

			if(!select.hasClass('user-selectlist-enroll')) {
				// 全ルーム一覧
				if(!option.prop('selected')) {
					selects = select.children('[data-parent-id='+roomId+']').prop('selected', false);
				} else {
					// 子グループ　親ルームも選択
					if(parent_id) {
						selects = select.children('[data-room-id='+parent_id+']:first').prop('selected', true);
					}
				}
				...
				if(!option.prop('selected')) {
					if(parent_id) {
						selects = select.children('[data-room-id='+parent_id+']:first').prop('selected', false);
					}
				} else {
					// 親ルーム　子グループも選択
					selects = select.children('[data-parent-id='+roomId+']').prop('selected', true);
				}
			}
			if(selects) {
 ...
 */
		successSelectAuth: function(userId, errorMes) {
			...
			setTimeout(function(){	// ヘッダーメッセージを表示するため
				if(errorMes != "") {
					// 警告メッセージを会員一覧に表示
					var description = $(".nc-top-description:first", tabList);
					if(description.prev().get(0) && description.prev().hasClass('nc-error-message')) {
						description.prev().remove();
					}
					description.before($.Common.unescapeHTML(errorMes));
				}
				active = tabs.tabs("option","active");
