...
;(function($) {
	$.fn.nc_insertvideo = function(options) {
		...
		function init() {
			...
			buttons = $('<div class="nc-btn-bottom"></div>').appendTo(div);

			ok = $('<input name="ok" type="button" class="nc-common-btn" value="'+__d(['nc_wysiwyg', 'dialog'], 'ok')+'" />').appendTo(buttons);
			cancel = $('<input name="cancel" type="button" class="nc-common-btn" value="'+__d(['nc_wysiwyg', 'dialog'], 'cancel')+'" />').appendTo(buttons);
