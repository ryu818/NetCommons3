 ...
 ;(function($) {
	$.fn.nc_tableproperty = function(options) {
		...
		function init() {
			...
			//ok cancel button
			buttons = $('<div class="nc-btn-bottom"></div>').appendTo( self );
			$(buttons).append($('<input name="ok" type="button" class="nc-common-btn" value="'+__d(['nc_wysiwyg', 'dialog'], 'ok')+'" />')
				.click(function(e){
					// 決定
			...
			$(buttons).append($('<input name="cancel" type="button" class="nc-common-btn" value="'+__d(['nc_wysiwyg', 'dialog'], 'cancel')+'" />')
				.click(function(e){
					options.wysiwyg.removeDialog("nc-wysiwyg-tableproperty");
					e.preventDefault();
			        return false;
