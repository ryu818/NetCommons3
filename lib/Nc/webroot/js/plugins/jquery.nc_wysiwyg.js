						...
						insertimage : { visible : true,
										...
										components : {
											insertimageDetail : function (e, img, popup_type) {
												var self = this;
												var plugin = self.options.plugin, ncBlock;
												if(!plugin) {
													ncBlock = $('#' + self.id).parents('.nc-block:first');
													if(!ncBlock.get(0)) {
														$.Common.alert(__('Can not get the plugin name.[e.g. javascript::nc_wysiwyg({plugin:XXXX}):]'), e);
														return;
													}
													plugin = ncBlock.attr('data-action').split("/")[0];
												}
												popup_type = (popup_type == undefined) ? 'image' : popup_type;
												var uploadOptions = {
													'plugin' : plugin,
													'el' : $('#' + self.id + '-btn-insert' + popup_type),
													'img' : img,
													'popup_type' : popup_type,
													'wysiwyg' : self,
													'multiple' : true
												};
												$.Common.showUploadDialog('dialog-'+self.id, uploadOptions);
											}
										}
						},
					...
					for (var node = root.firstChild; node; node = node.nextSibling) {
						...
						switch (node.nodeType) {
							...
							case 11: // Node.DOCUMENT_FRAGMENT_NODE
								...
								if($._nc.nc_wysiwyg['allow_js'] == 0 && (node_name == "script" || node_name == "object")) {
