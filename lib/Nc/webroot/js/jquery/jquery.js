...
// debug情報をbodyの最後かポップアップで送出するようにするため、jqueryを修正
	ncShowLog: function( data, url ) {
		...
		if($._debug != 0) {
			...
			var fatal_flag = false, content = '', log, logger_block = '', parentContainer = $('#parent-container');
			...
			if(content != '') {
				log = $('table.nc-log:first');
				if(log.get(0)) {
					log.before(content);
				} else if(parentContainer.get(0)) {
					parentContainer.append(content);
				} else {
					$(document.body).append(content);
				}
				if(fatal_flag) {
					data = data.replace(re_fatal,"");
				} else {
			...
			if(logger_block) {
				...
				log = $('table.nc-log:first');
				if(log.get(0)) {
					if(log.prev().hasClass('cake-error')) {
						log.prev().before(content);
					} else {
						log.before(content);
					}
				} else {
				...
				data = data.replace(re_sys_error,"");
			}
		}
		return data;
	},
...
// Add for NetCommons Extentions By Ryuji.M --START
										responses.text = $.ncShowLog(responses.text, s.url);
// Add for NetCommons Extentions By Ryuji.M --E N D
									}
...
// $.browserを使用しているプラグインが多くあるため、
// 一時、$.browserを追加($.supportを使用するように修正していく必要あり)
// (function() {
...
matched = jQuery.fn.uaMatch( navigator.userAgent );
browser = {};
