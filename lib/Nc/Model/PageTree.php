 ...
 */
	...
	public function addPage($pageId , $parentId) {
		...
		$ins  = $this->_createAddArray($pageId , $treeList);
	 ...
	 * @since   v 3.0.0.0
	 */
	private  function _createAddArray($pageId , $treeList) {
	...
	public function deletePage($pageId) {
		...
		//子のid情報を取得
		$childList = $this->_childIdsArray($this->getChildAll($pageId));
		//移動するノード以下の親子情報を取得
		...
		//再構築のため、一旦ノードごと削除する
		if(! $this->deleteNode($pageId)) return false;
	 ...
	 * @since   v 3.0.0.0
	 */
	private function getParentOneId($pageId) {
		//パラメータチェック
		if(! is_numeric($pageId) || ! $pageId) {
			return null;
		}
	 ...
	 * @since   v 3.0.0.0
	 */
	private  function _childIdsArray($pageTree) {
		//パラメータのチェック
	...
	public function deleteNode($pageId) {
		...
		$pageIds = $this->_childIdsArray($this->getChildAll($pageId));
	...
	public function moveNode($pageId , $newParentId) {
		...
		//新しい親に追加
		if(! $this->addPage($pageId , $newParentId)) return false;
