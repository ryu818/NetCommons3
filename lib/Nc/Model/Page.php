 ...
 */
	public function _duplicationPageName($check) {
		$positionFlag = intval($this->data['Page']['position_flag']);
		if ($positionFlag == _OFF) {
			return true;
		}
 ...
 */
	public function _notEmptyPermalink($check) {
		...
		if ($positionFlag == _OFF || $thread_num == 0) {
			// TOPノード
			if ($permalink != '') {
				return false;
			}
 ...
 */
	public function _invalidPermalink($check) {
		...
		$space_type = intval($this->data['Page']['space_type']);
		$permalinkArr = explode('/', $permalink);
		$chk_thread_num = ($space_type == NC_SPACE_TYPE_PUBLIC || ($thread_num == 2 && $display_sequence == 1)) ? $thread_num - 1 : $thread_num;
		if (count($permalinkArr) != $chk_thread_num) {
			return false;
		}
		$current_permalink = $permalinkArr[$chk_thread_num - 1];
 ...
 */
	...
	/**
	 * afterFindIdsで$typeがlistの場合
	 * @param $results
	 * @param $userId
	 * @param null $fields
	 * @return array
	 */
	public function _afterFIndIdsTypeList($results , $userId ,  $fields = null) {
		//初期設定
		...
		if (count($pages) == 0) return false; else return $pages;
	}
/**
 * afterFind
 * @param   array   $results
 ...
 */
	public function paginate($conditions, $fields, $order, $limit, $page = 1, $recursive = null, $extra = array()) {
		...
		return $this->findViewable('list', ($isAll) ? 'all' : $extra['user_id'], $addParams, $options);
	}
 ...
 */
	public function paginateCount($conditions = null, $recursive = 0, $extra = array()) {
		...
		return $this->findCommunityCount(($isAll) ? 'all' : $extra['user_id'], $params, $options);
	}
 ...
 */
	public function getMovePermalink($page, $parent_page) {
		...
		if ($parent_page['Page']['permalink'] != '') {
			$permalink = $parent_page['Page']['permalink'] . '/' . $permalinkArr[count($permalinkArr)-1];
		} else {
			$permalink = $permalinkArr[count($permalinkArr)-1];
		}
		return $permalink;
	}
 ...
 */
	public function insTopRoom($spaceType, $userId, $permalink, $authority = null) {
		...
		} elseif ($spaceType == NC_SPACE_TYPE_PRIVATE) {
			//private roomを作成する
			$pagePrivate = ClassRegistry::init('PagePrivate');
